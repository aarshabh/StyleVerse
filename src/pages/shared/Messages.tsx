...
const Messages = () => {
    ...
    const checkAuthAndFetchData = async () => {
      try {
        ...
        if (userData.data.role === "customer") {
          ...
          const requestsResponse = await requestService.getRequests({ customerId: userData.data.id })
          ...
          for (const request of userRequests) {
            if (request.designerId) {
              ...
              if (designer) {
                const messagesResponse = await chatService.getMessages(userData.data.id, designer.id)
        ...
        if (partners.length > 0) {
          ...
          const messagesResponse = await chatService.getMessages(userData.id, Number(partners[0].user.id))
  ...
  useEffect(() => {
    const loadMessages = async () => {
      ...
      try {
        const messagesResponse = await chatService.getMessages(Number(user.id), Number(selectedPartner.id))
        setMessages(messagesResponse.data)
      } catch (error) {

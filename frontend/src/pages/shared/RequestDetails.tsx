...
const RequestDetails = () => {
  ...
  const [request, setRequest] = useState<ProjectRequest | null>()
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('req, resp', requestResponse.data[0]);
        ...
        console.log('proposals', proposalsResponse);
    ...
    try {
      ...
      if (status === "delivered" && request) {
        try {
          ...
          // Update the backend
          await requestService.updateRequest(request.request_id, { status: "completed" });
        } catch (error) {
      ...
      <div className="flex-1 container mx-auto py-8 px-4 max-w-screen-xl">
        ...
        <div className="flex flex-col md:flex-row justify-between mb-6">
          ...
          {isAssignedDesigner && request.status === "assigned" && (
            <div className="mt-4 md:mt-0">
              <Button
                onClick={() => handleManageOrder(request.request_id)}
                className="bg-fashion-purple hover:bg-fashion-purple-dark"
              >
                Manage Order
              </Button>
                  ...
                  <ProposalCard
                    ...
                    onManageOrder={
                      isAssignedDesigner && proposal.status === "accepted"
                        ? () => handleManageOrder(request.request_id)
                        : undefined
                    }
                  />
                ))}

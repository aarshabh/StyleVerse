...
const pool = new Pool({
  ...
  database: process.env.POSTGRES_DB || "styleverse",
  password: process.env.POSTGRES_PASSWORD || "postgres",
  port: Number.parseInt(process.env.POSTGRES_PORT || "5431"),
})
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    const queryText = `
      INSERT INTO users (email, password, name, role, date_joined)
      VALUES ($1, $2, $3, $4, NOW())
      RETURNING user_id, email, name, role, date_joined
    `

    const queryParams = [email, hashedPassword, name, role]
...
app.get("/api/auth/me", authenticateToken, async (req, res) => {
  try {
    ...
    const result = await query("SELECT user_id, email, name, role, date_joined FROM users WHERE user_id = $1", [userId])
...
app.get("/api/users", authenticateToken, cacheMiddleware(300), async (req, res) => {
  try {
    ...
    let queryText = "SELECT user_id, email, name, date_joined, role FROM users"
    const queryParams = []
...
app.post("/api/users", authenticateToken, async (req, res) => {
  try {
    ...
    const { email, password, name, last_name, role = "Customer" } = req.body
...
// POST create a new customization request with or without file upload
app.post("/api/customization-requests", authenticateToken, upload.array("design_images", 5), async (req, res) => {
  console.log(req);
  try {
    const { title, description, material, budget, timeframe, images, additionalDetails, size } = req.body
    console.log(req.body);
    console.log('tryinggg', req.body);
    const user_id = ''
    // console.log(user_id, description, due_date, priority);
    const files = req.files || []

    if (!description) {
      return res.status(400).json({
        success: false,
        message: "Missing required fields. User ID and description are required.",
      })
    }
    ...
    // Insert each image URL into the customization_request_images table if we have any
...
app.get("/api/customization-requests/:requestId", async (req, res) => {
  try {
    const { requestId } = req.params;
    const queryText = "SELECT * FROM customization_requests WHERE request_id = $1"
    const result = await query(queryText, [requestId])
...
app.post("/api/customization-request-images", async (req, res) => {
  try {
    ...
    console.log(request_id, image_url, "post request images")
